...
**已知问题 — always_loaded skills 重复注入**:
`always_loaded` 类型的 skill 在上下文中出现两次：一次在 XML 摘要中（`to_context_xml()`），一次以完整内容追加。每次对话浪费 ~200 tokens。应在 XML 摘要中排除 `always_loaded` 或不再追加全文。

**`_identity_prompt()` 运行时信息**:
```
你是 MXWbot，一个智能助手。
## 运行时
...
## 行为准则
- 执行工具前先说明意图，但不要预测或声称工具结果
- 修改文件前先读取
- 工具调用失败时，分析错误原因再尝试不同方法
- ...
```
...
| 类 | 职责 |
|-----|------|
| `AgentRunSpec` | 单次 Agent 执行配置：provider、tools、max_iterations、checkpoint_interval、bus（确认通道）、hook、on_checkpoint 回调 |
    ...
    async def run(self, spec: AgentRunSpec, messages: list[dict]) -> AgentRunResult:
        ...
        while iteration < spec.max_iterations:
            ...
            if not response.is_ok:
                return AgentRunResult(finish_reason="error", ...)

            # 无工具调用 → 返回最终内容
            if not response.tool_calls:
                return AgentRunResult(content=response.content, finish_reason="stop", ...)

            # 有工具调用 — 累积计数
            tool_calls = response.tool_calls
            total_tool_calls += len(tool_calls)

            # 检查写操作 → 批量确认（一次 iteration 只弹一次）
            writable_tools = [tc for tc in tool_calls
            ...
            if writable_tools and spec.bus:
                ...
                if not approved:
                    return AgentRunResult(finish_reason="denied", ...)
            ...
            # 注入结果到消息列表（assistant + tool messages）
            messages.append(tool_calls formatted...)
            for cid, result in results:
                messages.append(tool result formatted...)

            # Hook: after tool calls（传入 tool_name 而非 call_id）
            if spec.hook:
                for cid, result in results:
                    tool_name = next(tc.name for tc in tool_calls if tc.id == cid)
        ...
        # 达到最大迭代次数
        return AgentRunResult(finish_reason="max_iterations", ...)
```

**finish_reason 枚举**:
| 值 | 含义 |
|----|------|
| `"stop"` | LLM 返回最终文本，正常结束 |
| `"error"` | LLM 返回 error response |
| `"denied"` | 用户拒绝了写操作确认 |
...
| 类 | 职责 |
|-----|------|
| `AgentHook` | 生命周期钩子基类：`on_turn_start` / `before_llm_call` / `on_stream_delta` / `on_llm_response` / `before_tool_call` / `after_tool_call` / `on_turn_end`。所有方法默认为 no-op，子类按需覆写 |
| `StreamProcessHook(AgentHook)` | 流式输出处理 Hook（think 标签清洗 + 增量计算）。`accumulated` 累积原始文本，`cleaned` 累积清洗后文本。`reset()` 清空状态准备下一轮 |

**AgentHook 生命周期**:
```
on_turn_start(session, state_manager, inbound_msg)       # 一轮对话开始
  → before_llm_call(messages, tools)                     # LLM 调用前，可修改 messages/tools
    → on_stream_delta(delta)                             # LLM 流式输出的增量片段
    → on_llm_response(response)                           # LLM 返回完整响应后
    → before_tool_call(name, args)                        # 某个工具执行前
    → after_tool_call(name, result)                       # 某个工具执行后
  → on_turn_end(session, response)                       # 一轮对话结束
```

**StreamProcessHook 流式处理**:
```python
class StreamProcessHook(AgentHook):
    def __init__(self, clean_thinking: bool = True):
    ...
    async def on_stream_delta(self, delta: str) -> None:
        ...
        if self._clean_thinking:
            # 对累积文本做 think 标签移除，取增量
            prev = "".join(self._cleaned)
            full = "".join(self._accumulated)
            clean = clean_think_tags(full)
            if clean != prev:
                self._cleaned.append(clean[len(prev):])
        else:
            self._cleaned.append(delta)

    @property
    def full_text(self) -> str:      # 累积的原始文本
    @property
    def cleaned_text(self) -> str:   # 清洗后的文本
    def reset(self) -> None:         # 清空累积状态
```
...
| 类 | 职责 |
|-----|------|
| `SkillLoader` | 扫描工作区 → 解析 YAML frontmatter → 生成 XML 技能摘要 → 按需获取完整内容 |
| `Skill` | 单个技能实体（name, description, path, content, availability, dependencies）|
...
dependencies: [git]
availability: always      # always | manual
---
...
**解析规则** — 缺失 frontmatter 或解析失败静默返回 None，不影响其他 skill 扫描：
...
**加载流程**:
1. `scan()` 遍历搜索目录中所有 `.md` 文件，解析 YAML frontmatter
2. `to_context_xml()` 生成 `<skills>` XML 摘要（仅 name+description+availability）
3. `get_always_loaded()` 返回 availability="always" 的技能全文
4. `get(name)` 按名称获取 Skill

**与设计计划的差异**:
- 省略了 `SkillMeta` 类（version / trigger_keywords / available 字段无消费方）
- 省略了懒加载（skill body 随 scan 一次性加载，.md 体积小无需惰性）
- 省略了 watchdog 热重载（后续可加）
- 缺失 frontmatter 静默返回 None 而非抛异常
...
class SubAgentConfig:
    task: str                         # 任务描述
    provider: LLMProvider             # 子代理使用的 LLM Provider
    tools: ToolRegistry | None = None # None = 无工具
    max_iterations: int = 5           # 最大迭代次数
    timeout_seconds: int = 120        # 总超时
    workspace: Path | None = None     # 工作区隔离（预留，当前未使用）
```
...
class SubAgentManager:
    ...
    async def spawn(self, config: SubAgentConfig) -> str:      # 返回 agent_id
    async def cancel(self, agent_id: str) -> bool:             # 取消正在运行的子代理
    async def wait(self, agent_id: str, timeout=None) -> SubAgentResult:
    async def wait_all(self) -> list[SubAgentResult]:          # 等待所有活跃子代理
    @property
    def active_count(self) -> int:
```

**与设计计划的差异**:
- 省略了 `SubAgentStatus` 枚举（用字符串 `"completed"/"cancelled"/"timeout"/"error"` 替代）
- 省略了 `SubAgentEvent` 事件日志（简化实现）
- 省略了 `status()` / `list_results()` 方法（`wait()` 返回结果即够用）
- 省略了 `_results` 结果缓存字典
- `SubAgentConfig.model` → `SubAgentConfig.provider`（直接注入 Provider 实例而非按名称查找）
- `SubAgentConfig.workspace` 字段存在但尚未使用（`_run_subagent()` 手工构建 system prompt，未通过 ContextBuilder）
- 子代理不继承父 Session 上下文（不注入 Skills/Memory，与 plan 一致）
...
| 文件 | 职责 |
|------|------|
| `base.py` | `Tool` 抽象基类 + `ToolResult` + `cast_params`/`validate_params`/`to_openai_schema` + 完整 JSON Schema 校验引擎 |
| `register.py` | `ToolRegistry`：注册/注销/查找/`get_definitions`(排序+缓存)/`prepare_call`/`execute_batch` |
| `filesystem.py` | `ReadFileTool` / `WriteFileTool` / `EditFileTool` / `ListDirTool` / `GlobTool` / `GrepTool`，workspace约束+`is_path_safe`安全边界 |
| `shell.py` | `ShellTool`：纵深防御（allow→deny→注入→workspace边界）→ sandbox 包装 → 进程管理（spawn/timeout/kill+WNOHANG/output截断） |
| `sandbox.py` | 可插拔后端注册表 + `wrap_command()` + `_bwrap()`(tmpfs parent掩藏/--ro-bind-try/--proc/--dev)；`SandboxTool` 为 LLM 入口 |
| `web.py` | `WebSearchTool`（DuckDuckGo）+ `WebFetchTool`（httpx GET） |
| `cron.py` | `CronTool`：add/list/remove + 相对/绝对时间解析 + JSONL 持久化 |
| `mcp.py` | `McpTool`：MCP 协议兼容（v1 存根） |
