...
def _resolve_in_workspace(path_str: str, workspace: Path) -> Path | None:
...
class ListDirTool(Tool):
    ...
    parameters = {
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "目录路径（相对于工作区，默认 '.'）"},
        },
    }
...
class GrepTool(Tool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "path": {"type": "string", "description": "搜索目录（相对路径，默认 '.'）"},
    ...
    async def execute(self, pattern: str, path: str = ".", glob: str = "*", **kwargs: Any) -> ToolResult:
        ...
        search_dir = _resolve_in_workspace(path, self._workspace)
