...
| 文件 | 职责 | 核心类/函数 |
|------|------|-------------|
| `base.py` | 定义 `LLMProvider` 抽象基类、`LLMResponse`、`LLMStreamChunk`、`ToolCallDelta`、`ToolCallRequest`、`TokenUsage`、`LLMCallPurpose` 枚举 | `LLMProvider`, `LLMResponse`, `LLMStreamChunk`, `ToolCallDelta`, `ToolCallRequest`, `TokenUsage`, `LLMCallPurpose` |
...
@dataclass
class ToolCallDelta:
    """流式工具调用的增量（跨多个 chunk 累积）"""
    index: int              # 对应第几个工具调用
    id: str | None = None   # 首个 chunk 有 id
    name: str | None = None # 首个 chunk 有 name
    arguments_delta: str    # 累积的参数 JSON 片段（由 Hook 层聚合）
```
...
| 文件 | 职责 |
|------|------|
| `messages.py` | `InboundMessage`（含 `msg_type` / `idempotency_key`）+ `OutboundMessage`（含 `msg_type` / 确认字段）+ `StreamDelta`（含 `error`） |
...
class OutboundMessage:
    ...
    chat_id: str
    content: str
    reply_to: str | None = None        # 关联原始 InboundMessage.id
    # 确认类消息专用
...
- Compact 重写场景：临时文件 + `os.replace()` 原子替换，崩溃时原始文件完好
  ```python
  async def compact(self, session, keep_last=200):
      tmp_path = session.path.with_suffix(".jsonl.tmp")
      try:
          async with aiofiles.open(tmp_path, "w") as f:
              for msg in session.messages[-keep_last:]:
                  await f.write(json.dumps(msg, ensure_ascii=False) + "\n")
          os.replace(tmp_path, session.path)  # 原子替换
      except Exception:
          tmp_path.unlink(missing_ok=True)     # 清理临时文件
          raise
  ```
- 文件名校验：`channel_chat_id.jsonl`，防止路径穿越
- 单 Session 文件最大 1000 条消息，超出自动 compact（保留最近 200 条）
